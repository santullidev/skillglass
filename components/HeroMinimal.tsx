...
export default function HeroMinimal({ images, metadata, title, subtitle, ctaTexto, ctaLink }: Props) {
  ...
  return (
    <section className="relative h-screen flex items-center justify-center overflow-hidden bg-surface-lowest">
      {/* Background Images - Fully visible (100% opacity) */}
      <div className="absolute inset-0 z-0">
        {images && images.length > 0 ? (
          ...
          /* Fallback with the refraction look */
          <div className="absolute inset-0 bg-white">
            <div 
              className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] rounded-full opacity-20 blur-[120px]"
              style={{ background: 'radial-gradient(circle, #FAD0C4 0%, #FFD1FF 100%)' }}
            />
            <div 
              className="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] rounded-full opacity-20 blur-[150px]"
              style={{ background: 'radial-gradient(circle, #E0C3FC 0%, #8EC5FC 100%)' }}
            />
          </div>
        )}
      ...
      <div className="relative z-30 text-center px-4 md:px-6 max-w-5xl mx-auto flex flex-col items-center">
        <div className="glass-panel p-10 md:p-16 lg:p-24 flex flex-col items-center max-w-4xl shadow-2xl border-white/40">
          ...
          {/* Title - Gradient Text */}
          <h1 
            className="text-gradient-gold mb-6 md:mb-8 leading-tight tracking-tighter drop-shadow-sm"
          ...
          {/* Subtitle */}
          <p className="text-on-surface-variant text-lg md:text-xl lg:text-2xl max-w-2xl font-serif italic mb-12 text-balance leading-relaxed">
      ...
      {/* Tech Metadata Overlays (Outside the box) */}
