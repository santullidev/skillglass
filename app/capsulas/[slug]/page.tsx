...
export default async function ColeccionDetailPage({ 
  ...
  return (
    <div className="min-h-screen pt-32 pb-24 px-6 lg:px-8 max-w-7xl mx-auto">
      {/* Breadcrumbs */}
      <nav className="max-w-7xl mx-auto px-6 lg:px-8 py-6 mb-8 mt-24">
        <div className="flex items-center text-sm text-on-surface-variant font-mono uppercase tracking-widest gap-2">
          <Link href="/" className="hover:text-primary transition-colors">Inicio</Link>
          <span className="text-white/20">/</span>
          <Link href="/capsulas" className="hover:text-primary transition-colors">Cápsulas</Link>
          <span className="text-white/20">/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center mb-24 relative">
        <div className="relative z-10">
          <p className="label-text mb-4 text-primary">CÁPSULA EXCLUSIVA</p>
        ...
        {coleccion.imagenes && coleccion.imagenes.length > 0 && (
          <div className="mb-24 aspect-[16/9] lg:aspect-[21/9] w-full relative">
            <ImageCarousel images={coleccion.imagenes} alt={`Cápsula ${coleccion.nombre}`} />
